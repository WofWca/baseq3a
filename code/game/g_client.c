...
//	char		*areabits;
	...
	// Free them so that the slots become available for actual entities
	// (see `G_SpawnFromEntPool`).
	if ( level.mustFreeEntPoolEnts ) {
		G_Printf( "entity pools' baseline state set, now freeing them all\n" );
		EntPoolsFreeEnts();
		level.mustFreeEntPoolEnts = qfalse;
	}
#endif
