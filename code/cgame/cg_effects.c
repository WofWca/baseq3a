...
void CG_GibPlayer( const vec3_t playerOrigin, const vec3_t playerVelocity ) {
	vec3_t	origin, velocity;
	float baseGibVelocityScale = 0.5;

	float directionalVelocityScale = cg_gibsDirectionalVelocityScale.value;

	if ( !cg_blood.integer ) {
		return;
	}

	// TODO do we need to handle dir being `NULL`?

	// TODO also adjust speeds? So that gibs don't fly inwards.

	VectorCopy( playerOrigin, origin );
	// Viewheight instead of maxs because it more closely corresponds
	// to the head position.
	origin[2] += DEFAULT_VIEWHEIGHT;
	velocity[0] = (crandom()*GIB_VELOCITY)*baseGibVelocityScale;
	velocity[1] = (crandom()*GIB_VELOCITY)*baseGibVelocityScale;
	velocity[2] = (GIB_JUMP + crandom()*GIB_VELOCITY)*baseGibVelocityScale;
	VectorMA(velocity, directionalVelocityScale, playerVelocity, velocity);
	if ( rand() & 1 ) {
